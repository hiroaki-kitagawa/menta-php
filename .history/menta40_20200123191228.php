...
const REMATCH_ANSWER = [
    ONEMORE => '1:対戦する' . PHP_EOL,
    NOMORE => '2:やめる' . PHP_EOL
];
...
function janken() {
    ...
    $answer = rematch();
    if ($answer === ONEMORE) {
        echo REMATCH_ANSWER[ONEMORE];
        janken();
    } elseif ($answer === NOMORE) {
        echo REMATCH_ANSWER[NOMORE];
        endgame();
    }
}
...
function validateHand($hand) {
    validateBasic($hand);
    // if ($hand < 1 || $hand > 3) {
    if (array_key_exists(REMATCH_ANSWER[$hand])) {
